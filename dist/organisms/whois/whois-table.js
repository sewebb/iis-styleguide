...
const _tooltip = require("../../atoms/tooltip/tooltip");
...
const renderTooltip = (tooltip)=>{
    ...
    return `<button type="button" class="${(0, _className.default)('a-tooltip a-tooltip--small a-button a-button--standalone-icon a-button--icon')}" aria-label="${escapeHtml(label)}" data-tooltip-placement="${escapeHtml(placement)}">
		<span class="${(0, _className.default)('a-tooltip__text')}" hidden>${escapeHtml(tooltip.text)}</span>
		<svg class="icon ${(0, _className.default)('a-button__icon')}" aria-hidden="true">
...
const renderLink = (link)=>{
    ...
    const icon = link.external ? `<svg class="icon ${(0, _className.default)('o-mega-menu__link__icon')}"><use xlink:href="#icon-external-link"></use></svg>` : '';
    return `<a href="${escapeHtml(link.href)}"${target}>${escapeHtml(link.text)}${icon}</a>`;
};
const renderButton = (button)=>{
    if (!button) return '';
    const dataAttrs = Object.entries(button.data || {}).map(([key, value])=>` data-${escapeHtml(key)}="${escapeHtml(value)}"`).join('');
    const icon = button.icon ? `<svg class="icon ${(0, _className.default)('a-button__icon')}" aria-hidden="true"><use xlink:href="${escapeHtml(button.icon)}"></use></svg>` : '';
    return `<button type="button"${dataAttrs} class="${(0, _className.default)('a-button a-button--lemon-light a-button--small a-button--icon')} whois-button u-nowrap">
		<span class="${(0, _className.default)('a-button__text')}">${escapeHtml(button.text)}</span>
		${icon}
	</button>`;
};
const renderWhoisCell = (params)=>{
    ...
    const parts = [];
    if (value.label) {
        const tooltip = value.tooltip ? `<span class="whois-cell__inline-tooltip">${renderTooltip(value.tooltip)}</span>` : '';
...
function getColumnApi(params) {
    return params.columnApi || (params.api && typeof params.api.getColumnApi === 'function' ? params.api.getColumnApi() : null);
}
...
function getWhoisGroups(data) {
    return data.filter((row)=>{
        var _row_section, _row_section1;
        return ((_row_section = row.section) == null ? void 0 : _row_section.id) && ((_row_section1 = row.section) == null ? void 0 : _row_section1.label);
    }).map((row)=>({
            id: row.section.id,
...
function buildWhoisMatrixRows(data, groups) {
    ...
    return Array.from({
        length: maxRows
    }, (_, index)=>groups.reduce((row, group)=>{
            const item = rowsByGroup[group.id][index];
            row[`${group.id}Field`] = (item == null ? void 0 : item.field) || null;
            row[`${group.id}Value`] = (item == null ? void 0 : item.value) || null;
            return row;
        }, {}));
}
...
function createGroupSwitcher(el, groups, activeGroup, onChange) {
    var _wrapper_previousElementSibling;
    const wrapper = el.closest('.js-ag-grid-wrapper');
    if (!wrapper || ((_wrapper_previousElementSibling = wrapper.previousElementSibling) == null ? void 0 : _wrapper_previousElementSibling.classList.contains('js-whois-table-switcher'))) {
        return (wrapper == null ? void 0 : wrapper.previousElementSibling) || null;
    }
    const switcher = document.createElement('div');
    switcher.className = 'whois-table-switcher js-whois-table-switcher';
    switcher.setAttribute('aria-label', 'Välj grupp');
    switcher.innerHTML = groups.map((group)=>`<button type="button" class="whois-table-switcher__button js-whois-table-group" data-whois-table-group="${escapeHtml(group.id)}" aria-pressed="${group.id === activeGroup.id ? 'true' : 'false'}">${escapeHtml(group.label)}</button>`).join('');
...
document.addEventListener('tableAdvanced:configure', (event)=>{
    ...
    const activeGroup = {
        id: (_groups_ = groups[0]) == null ? void 0 : _groups_.id
    };
