...
function setupTablist(tablist) {
    const tabs = Array.from(tablist.children).filter((element)=>element.matches(TAB_SELECTOR));
    if (!tabs.length) {
        return;
    }
    tablistCounter += 1;
    ...
    tablist.addEventListener('keydown', (event)=>{
        ...
        if (event.key === 'ArrowRight' && orientation === 'horizontal' || event.key === 'ArrowDown' && orientation === 'vertical') {
        ...
        if (event.key === 'ArrowLeft' && orientation === 'horizontal' || event.key === 'ArrowUp' && orientation === 'vertical') {
            nextIndex = currentIndex - 1;
        }
        if (nextIndex !== null) {
