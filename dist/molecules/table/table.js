...
function getHeaderLabels(table) {
    return Array.from(table.querySelectorAll('thead th')).map((cell)=>cell.textContent.replace(/\s+/g, ' ').trim()).filter(Boolean);
}
...
function enhanceScrollableTable(wrapper) {
    ...
    const container = wrapper.querySelector(TABLE_CONTAINER_SELECTOR);
    const table = container == null ? void 0 : container.querySelector(TABLE_SELECTOR);
    if (!container) {
        return;
    }
