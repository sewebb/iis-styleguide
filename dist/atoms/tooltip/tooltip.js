...
const ROOT_SELECTOR = `.${(0, _className.default)('a-tooltip')}`;
const TOOLTIP_SELECTOR = `.${(0, _className.default)('a-tooltip__text')}`;
...
const supportsAnchorPositioning = typeof CSS !== 'undefined' && CSS.supports('anchor-name: --tooltip-anchor') && CSS.supports('position-anchor: --tooltip-anchor') && CSS.supports('top: anchor(top)');
...
function getPlacementScore(placement, triggerRect, tooltipRect, viewport) {
    ...
    const inlineOverflow = placement === 'top' || placement === 'bottom' ? Math.max(0, tooltipRect.width / 2 - (centerX - VIEWPORT_PADDING)) + Math.max(0, tooltipRect.width / 2 - (viewport.width - VIEWPORT_PADDING - centerX)) : Math.max(0, tooltipRect.height / 2 - (centerY - VIEWPORT_PADDING)) + Math.max(0, tooltipRect.height / 2 - (viewport.height - VIEWPORT_PADDING - centerY));
    const blockSpace = {
    ...
    const requiredSpace = placement === 'top' || placement === 'bottom' ? tooltipRect.height + TOOLTIP_GAP : tooltipRect.width + TOOLTIP_GAP;
...
function choosePlacement(preferredPlacement, triggerRect, tooltipRect) {
    ...
    return PLACEMENT_FALLBACKS[preferredPlacement].map((placement)=>({
            placement,
...
class Tooltip {
    setupAccessibility() {
        ...
        if (!this.root.hasAttribute('aria-label') && !this.root.hasAttribute('aria-labelledby') && !hasOwnVisibleText(this.root)) {
            this.root.setAttribute('aria-label', this.root.getAttribute('data-tooltip-label') || 'Visa mer information');
        }
    ...
    onTriggerFocusOut(event) {
        if (this.isPinned || this.root.contains(event.relatedTarget) || this.tooltip.contains(event.relatedTarget)) {
            return;
        }
        this.scheduleHide();
    }
    ...
    scheduleHide() {
        ...
        this.hideTimer = window.setTimeout(()=>{
            if (this.isPinned || this.isHoveringTrigger || this.isHoveringTooltip || this.root.matches(':focus-within')) {
                return;
            }
            this.hide();
        }, HIDE_DELAY);
    ...
    updatePlacement() {
        ...
        const viewport = getViewportSize();
        const placement = choosePlacement(this.preferredPlacement, triggerRect, tooltipRect);
        const centerX = triggerRect.left + triggerRect.width / 2;
        const centerY = triggerRect.top + triggerRect.height / 2;
        const isVertical = isVerticalPlacement(placement);
        this.tooltip.dataset.placement = placement;
        if (!supportsAnchorPositioning) {
            const idealCrossAxisOffset = isVertical ? centerX - tooltipRect.width / 2 : centerY - tooltipRect.height / 2;
            const maxCrossAxisOffset = isVertical ? viewport.width - tooltipRect.width - VIEWPORT_PADDING : viewport.height - tooltipRect.height - VIEWPORT_PADDING;
            const clampedCrossAxisOffset = clamp(idealCrossAxisOffset, VIEWPORT_PADDING, maxCrossAxisOffset);
            const top = placement === 'top' ? triggerRect.top - tooltipRect.height - TOOLTIP_GAP : placement === 'bottom' ? triggerRect.bottom + TOOLTIP_GAP : clampedCrossAxisOffset;
            const left = placement === 'left' ? triggerRect.left - tooltipRect.width - TOOLTIP_GAP : placement === 'right' ? triggerRect.right + TOOLTIP_GAP : clampedCrossAxisOffset;
            ...
            const arrowOffset = clamp(isVertical ? centerX - clampedLeft : centerY - clampedTop, ARROW_OFFSET, (isVertical ? tooltipRect.width : tooltipRect.height) - ARROW_OFFSET);
        ...
        const idealOffset = isVertical ? centerX - tooltipRect.width / 2 : centerY - tooltipRect.height / 2;
        const maxOffset = isVertical ? viewport.width - tooltipRect.width - VIEWPORT_PADDING : viewport.height - tooltipRect.height - VIEWPORT_PADDING;
        const clampedOffset = clamp(idealOffset, VIEWPORT_PADDING, maxOffset);
        const arrowOffset = clamp((isVertical ? centerX : centerY) - clampedOffset, ARROW_OFFSET, (isVertical ? tooltipRect.width : tooltipRect.height) - ARROW_OFFSET);
