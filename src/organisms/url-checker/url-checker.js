...
function getScriptLabelByCodePoint(codePoint) {
	for (const pattern of SUSPICIOUS_SCRIPT_PATTERNS) {
		const isWithinRange = pattern.ranges.some(
...
function buildHostVisualMarkup(hostname) {
	...
	const flushBuffer = () => {
		...
		if (activeLabel) {
			...
			chunks.push(
				`<span class="${CLASS.breakdownSegment} ${CLASS.hostSegment} ${CLASS.hostSegmentSpecial}" title="${title}">${escaped}</span>`,
			);
		} else {
			chunks.push(escaped);
		}
	...
	for (const char of host) {
		...
		const label =
			codePoint === undefined ? '' : getScriptLabelByCodePoint(codePoint);
