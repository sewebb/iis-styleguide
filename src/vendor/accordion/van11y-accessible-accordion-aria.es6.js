	...
	const attach = (node) => {
		$listAccordions(node)
			.forEach((accordion_node) => {
				...
				const prefixClassName = accordion_node.hasAttribute(CONFIG.ACCORDION_DATA_PREFIX_CLASS) === true ? `${accordion_node.getAttribute(CONFIG.ACCORDION_DATA_PREFIX_CLASS)}-` : '';
				const coolSelectors = accordion_node.hasAttribute(CONFIG.ACCORDION_DATA_COOL_SELECTORS) === true;
				...
				const $listAccordionsHeader = [].slice.call(accordion_node.querySelectorAll(`.${CONFIG.ACCORDION_JS_HEADER}`));
				$listAccordionsHeader
					.forEach((header_node, index_header) => {
						...
						const accordionOpenedAttribute = header_node.hasAttribute(CONFIG.ACCORDION_DATA_OPENED) === true ? header_node.getAttribute(CONFIG.ACCORDION_DATA_OPENED) : '';
		...
		.forEach((eventName) => {
			document.body
				.addEventListener(eventName, (e) => {
					const hashId = searchParentHashId(e.target, DATA_HASH_ID); // e.target.dataset.hashId;
					...
					if (hashId !== '') {
						...
						if (hasClass(e.target, CONFIG.ACCORDION_JS_HEADER) === true && eventName === 'focus') {
							...
							const coolSelectors = accordionContainer.hasAttribute(CONFIG.ACCORDION_DATA_COOL_SELECTORS) === true;
						...
						if (hasClass(e.target, CONFIG.ACCORDION_JS_HEADER) === true && eventName === 'keydown') {
							const buttonTag = e.target;
							const idAccordionContainer = searchParent(buttonTag, CONFIG.ACCORDION_JS, hashId);
							const accordionContainer = findById(idAccordionContainer, hashId);
